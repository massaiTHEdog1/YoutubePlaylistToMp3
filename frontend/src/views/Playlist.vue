<template>
 	<b-container>
		...
		<playlist-item v-for="item in playlistItems" v-bind:key="item.Id" v-bind:playlist-item="item" v-on:downloadClick="OnDownloadClick"/>

		<b-button v-if="nextPageToken && !isLoading" style="margin: 10px auto; display: block;" @click="loadPlaylist(nextPageToken)">Load more</b-button>
		<font-awesome-icon v-if="isLoading" icon="spinner" spin size="lg" style="margin: 10px auto; display: block;"/>

		<b-modal id="modal-download" title="Download"
			...
			<img :src="thumbnailUrl" style="width: 140px; margin: auto; display: block;"/>
...
export default class Playlist extends Vue {
	...
	async startDownload(event : any, item : PlaylistItemDto, title : string, artist : string, url : string)
	{
